...
const ThoughtMap = () => {
  ...
  return (
    <div ref={mapRef} className="relative w-full h-screen flex items-center justify-center">
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none">
        {thoughtBubbles.map((bubble, index) => (
          <path
            ...
            d={`M ${typeof window !== "undefined" ? window.innerWidth / 2 : 400} ${typeof window !== "undefined" ? window.innerHeight / 2 : 300} Q ${
              (typeof window !== "undefined" ? window.innerWidth / 2 : 400) + bubble.position.x / 2
            } ${(typeof window !== "undefined" ? window.innerHeight / 2 : 300) + bubble.position.y / 2} ${
              (typeof window !== "undefined" ? window.innerWidth / 2 : 400) + bubble.position.x
            } ${(typeof window !== "undefined" ? window.innerHeight / 2 : 300) + bubble.position.y}`}
            fill="none"
            stroke="#C5B4E3"
            strokeWidth="2"
            strokeDasharray="5,5"
      ...
      <div
        ref={centralNodeRef}
        className="relative z-10 bg-champagne-beige border-4 border-charcoal rounded-full p-8 max-w-md text-center shadow-lg transform hover:scale-105 transition-transform duration-300"
        style={{
          borderStyle: "dashed",
          filter: "drop-shadow(3px 3px 6px rgba(0,0,0,0.1))",
        }}
      >
        <h1 className="font-caveat text-4xl font-bold text-charcoal mb-4">SNiFF</h1>
      ...
      {thoughtBubbles.map((bubble, index) => (
        <Button
          key={bubble.id}
          ref={(el) => {
            if (el) bubblesRef.current[index] = el as HTMLDivElement
          }}
          onClick={() => handleBubbleClick(bubble.id)}
          className="absolute z-10 bg-muted-lavender hover:bg-muted-lavender/80 border-2 border-charcoal rounded-full p-6 text-center shadow-lg transition-all duration-300 hover:scale-110 cursor-pointer group"
          style={{
        ...
        >
          ...
          <div className="font-patrick-hand text-sm font-semibold text-charcoal whitespace-nowrap">{bubble.label}</div>
        </Button>
      ))}
      ...
      <div className="absolute inset-0 pointer-events-none">
        {[...Array(20)].map((_, i) => (
          <div
            key={i}
            className="absolute w-1 h-1 bg-muted-lavender rounded-full opacity-30"
            style={{
