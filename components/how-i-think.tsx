    ...
    <div ref={sectionRef} className="container mx-auto px-6 py-20">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-16">
          <h2 className="font-caveat text-6xl font-bold text-charcoal mb-6">How I Think</h2>
          <p className="font-patrick-hand text-xl text-charcoal/80 max-w-2xl mx-auto mb-8">
          ...
          <Button
            ...
            className="bg-muted-lavender hover:bg-muted-lavender/80 text-charcoal border-2 border-charcoal font-patrick-hand"
          >
        ...
        <div
          ref={boardRef}
          className="relative h-96 bg-cream-white border-4 border-charcoal rounded-3xl p-8 mb-16 overflow-hidden"
          style={{
            borderStyle: "dashed",
        ...
        >
          {thoughtCards.map((card, index) => (
            <div
              key={card.id}
              className="thought-card absolute bg-champagne-beige border-3 border-charcoal rounded-2xl p-6 w-64 cursor-move hover:shadow-lg transition-shadow duration-300 group"
              style={{
            ...
            >
              <div className="text-center mb-4">
                ...
                <h3 className="font-caveat text-xl font-bold text-charcoal">{card.title}</h3>
              </div>
              <p className="font-patrick-hand text-sm text-charcoal leading-relaxed">{card.insight}</p>
              <div className="mt-4 text-center">
                <span className="font-patrick-hand text-xs text-charcoal/60">Click to expand</span>
              </div>
            </div>
          ))}
        </div>
        ...
        {selectedCard && (
          <Card className="border-3 border-charcoal bg-cream-white shadow-lg">
            <CardHeader>
              <CardTitle className="font-caveat text-3xl text-charcoal flex items-center gap-4">
                <span className="text-4xl">{thoughtCards.find((c) => c.id === selectedCard)?.sketch}</span>
                {thoughtCards.find((c) => c.id === selectedCard)?.title}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <p className="font-patrick-hand text-lg text-charcoal leading-relaxed mb-4">
                {thoughtCards.find((c) => c.id === selectedCard)?.insight}
              </p>
              <div className="border-t-2 border-muted-lavender pt-4">
                <p className="font-patrick-hand text-base text-charcoal/80 leading-relaxed">
        ...
        <div className="mt-16 text-center">
          <div className="bg-muted-lavender border-3 border-charcoal rounded-3xl p-8 max-w-3xl mx-auto">
            <h3 className="font-caveat text-3xl font-bold text-charcoal mb-4">The Core Philosophy</h3>
            <p className="font-patrick-hand text-lg text-charcoal leading-relaxed">
