    ...
    <div ref={sectionRef} className="container mx-auto px-6 py-20">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-16">
          <h2 className="font-caveat text-6xl font-bold text-charcoal mb-6">Projects</h2>
        ...
        <div ref={projectsGridRef} className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          {projects.map((project, index) => (
            <div
              key={project.id}
              className={`project-bubble ${project.color} border-3 border-charcoal rounded-3xl p-8 cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-lg relative overflow-hidden`}
            ...
            >
              ...
              <h3 className="font-caveat text-2xl font-bold text-charcoal mb-2">{project.title}</h3>

              <p className="font-patrick-hand text-sm text-charcoal/70 mb-4">{project.subtitle}</p>

              <p className="font-patrick-hand text-base text-charcoal leading-relaxed italic">
                "{project.description}"
              </p>

              {/* Hover indicator */}
              <div className="absolute bottom-4 right-4 opacity-50">
                <span className="text-sm font-patrick-hand">Click to expand</span>
              </div>
            </div>
          ))}
        </div>
          ...
          <Card className="border-3 border-charcoal bg-cream-white shadow-lg">
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="font-caveat text-3xl text-charcoal">
                    {projects.find((p) => p.id === selectedProject)?.title}
                  </CardTitle>
                  <CardDescription className="font-patrick-hand text-lg text-charcoal/70">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="flex flex-wrap gap-2">
                {projects
                  ...
                  ?.tech.map((tech) => (
                    <span
                      key={tech}
                      className="bg-muted-lavender text-charcoal px-3 py-1 rounded-full border-2 border-charcoal font-patrick-hand text-sm"
                    >
                      {tech}
                    </span>
                  ))}
              ...
              <div className="flex gap-4">
                <Button
                  variant="outline"
                  className="border-2 border-charcoal hover:bg-muted-lavender font-patrick-hand bg-transparent"
                  asChild
                >
                  <a
                ...
                <Button className="bg-charcoal hover:bg-charcoal/80 text-cream-white font-patrick-hand" asChild>
                  <a
